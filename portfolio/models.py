...
class PortfolioBase(models.Model):
    ...
    def get_absolute_url(self):
        return reverse(
            "portfolio-%s-detail" % self.__class__.__name__.lower(),
            args=(),
...
class Testimonial(models.Model):
    ...
    witness_desc = models.CharField(
        _("witness' position"),
        max_length=255,
        blank=True,
        help_text=_("Describe their position or relationship."),
    )
...
class ProjectPiece(SingleGFK):
    project = models.ForeignKey(
        "Project", related_name="pieces", on_delete=models.CASCADE
    )
...
class Project(PortfolioBase):
    ...
    completion_date = models.DateField(
        _("completion date"), blank=True, null=True
    )
    ...
    overview_image = models.ImageField(
        _("overview image"), blank=True, upload_to=_get_upload_to_path
    )
    detail_image = models.ImageField(
        _("detail image"), blank=True, upload_to=_get_upload_to_path
    )

    media = models.ManyToManyField(
        Medium, verbose_name=_("media"), related_name="projects"
    )
    disciplines = models.ManyToManyField(
        Discipline, verbose_name=_("disciplines"), related_name="projects"
    )
    status = StatusField(_("status"))
